  ...
  WHERE
    ...
    ( 'BEL084', 'CIV033', 'COL003', 'ETH013', 'IND002', 'KEN056', 'MEX002', 'NGA039', 'PER001', 'PHL001', 'SYR002', 'TWN001', 'FJI049', 'SWE054' )
      ...
      SELECT
        ...
        'per'                                                AS um
      FROM
        ( SELECT *, CASE WHEN threatened THEN 1 ELSE 0 END AS threatened_inc
          FROM answer_1_2 WHERE species IS NOT NULL ) raw
      GROUP BY
        iteration,
        country_iso3
      UNION
      ...
      FROM
        ( SELECT *, CASE WHEN threatened THEN 1 ELSE 0 END AS threatened_inc
          FROM answer_1_2 WHERE species IS NOT NULL) raw
        LEFT JOIN codelist.ref_region_country b on (raw.country_iso3 = b.country_iso3 )
      GROUP BY
        iteration,
        wiews_region,
        rank
